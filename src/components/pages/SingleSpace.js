...
const SingleSpace = props => {
  const { getSpace } = useContext(SpacesContext);
  ...
  const space = getSpace(initialState.slug);

  if (!space) {
    return (
      <div className='container text-center single-space-err'>
        <h2 className='lead'>
          <span className='text-danger'>Error!&nbsp;</span> This space could not
          be found!
        </h2>
  ...
  return (
    <Fragment>
      <StyledHero hero={mainImg || defaultImg}>
